...
class GhRepoScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider<RepoBloc>(
        create: (_) => getIt(),
        child: BlocBuilder<RepoBloc, RepoState>(builder: (context, state) {
          return RefreshStatefulScaffold<
            ...
            fetch: () async {
              ...
              final countFuture =
                  context.read<RepoBloc>().getContributorsCount(owner, name);

              final readmeFactory = (String acceptHeader) {
                return () {
                  return context
                      .read<RepoBloc>()
                      .getReadme(owner, name, acceptHeader)
                      .then((res) {
                    return res.body;
                  }).catchError((err) {
                    return null;
                          ...
                          MutationButton(
                            ...
                            onTap: () async {
                              ...
                              if (repo.viewerHasStarred) {
                                await activityApi.unstar(RepositorySlug(
                                    repo.owner.login, repo.name));
                              } else {
                                await activityApi.star(RepositorySlug(
                                    repo.owner.login, repo.name));
                              }
                              setData(data.withItem1(repo.rebuild((b) {
                    ...
                    items: [
                      ...
                      if (ref != null) ...[
                            ...
                            onTap: () async {
                              ...
                              await theme.showPicker(
                                ...
                                PickerGroupItem(
                                  ...
                                  items: refs
                                      .map((b) =>
                                          PickerItem(b.name, text: b.name))
                                      .toList(),
                                  onClose: (ref) {
                                    if (ref != branch) {
                                      theme.push(context,
                                          '/github/$owner/$name?ref=$ref',
                                          replace: true);
                                    }
                                  },
                                ),
                        ...
                        TableViewItem(
                          ...
                          rightWidget:
                              Text(repo.releases.totalCount.toString()),
                        ),
                      ],
                    ],
                  ),
